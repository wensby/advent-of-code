...
def run(input):
  ...
  robot.run()
  return render(paint_by_tile)

def parse_program(input):
  return list(map(int, input.split(',')))

def render(tiles):
  left = min(map(lambda x: x[0], tiles.keys()))
  right = max(map(lambda x: x[0], tiles.keys()))
  top = max(map(lambda x: x[1], tiles.keys()))
  bottom = min(map(lambda x: x[1], tiles.keys()))
  rows = []
  for y in range(bottom, top+1):
    row = []
    for x in range(left, right+1):
      color = '█' if tiles.get((x, y), 0) else ' '
      row.append(color)
